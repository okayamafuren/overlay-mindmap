...
첫번째 방법: release사용
  ...
  1단계: 앱을 바탕화면으로 꺼내기 (가장 중요)
지금 열려 있는 Overlay Mindmap 1.0.0 창 안에 있는 앱 아이콘을 마우스로 잡아서 **바탕화면(Desktop)**으로 끌어다 놓으세요(복사).

2단계: 바탕화면에 있는 앱에 권한 주기
앱을 바탕화면으로 옮기셨다면, 터미널에 아래 명령어를 복사해서 붙여넣기 하세요. (드래그 앤 드롭을 활용하면 정확합니다.)

터미널에 chmod +x 를 입력합니다 (뒤에 한 칸 띄우기).

바탕화면에 있는 'Overlay Mindmap.app' 아이콘을 터미널 창으로 끌어다 놓습니다.

엔터를 칩니다.

3단계: 보안 격리 해제 (sudo 명령어)
여전히 실행되지 않는다면, 맥이 인터넷에서 다운로드한 파일로 인식해 막고 있는 것입니다. 아래 명령어를 입력하세요.

Bash
sudo xattr -rd com.apple.quarantine ~/Desktop/Overlay\ Mindmap.app
